...
class ZVoxelExtension_CarH : public ZVoxelExtension
{
  ...
  ZVoxelExtension_CarH()
  {
    ULong i;
    ExtensionType = Extension_CarH;
    for (i=0;i<Storage_NumSlots;i++) {VoxelType[i] = 0; VoxelQuantity[i] = 0;}
  }

  virtual ZVoxelExtension * GetNewCopy()
  {
    ZVoxelExtension_CarH * NewCopy;
    NewCopy = new ZVoxelExtension_CarH(*this);
    return(NewCopy);
  }

  virtual ULong GetExtensionID()
  {
    return( MulticharConst('C','A','R','H') );
  }
