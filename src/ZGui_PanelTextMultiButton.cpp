...
*/
      ...
      ULong TextureRef = GuiManager->TextureManager->GetTextureEntry((*DataRef==ButtonNumber)? AlternateTextureNum:TextureNum)->OpenGl_TextureRef;
...
Bool ZPanelTextMultiButton::MouseButtonClick  (UShort nButton, Short Absolute_x, Short Absolute_y)
{
  if (nButton ==1)
  {
    if (DataRef) *DataRef = ButtonNumber;
  }

  return(ZFrame::MouseButtonClick(nButton,Absolute_x, Absolute_y));
}
